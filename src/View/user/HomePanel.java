...
public class HomePanel extends FatherJPanel {
    ...
    private JLabel[] cabinetsLabels;      //充电柜标签
    ...
    // 封装所有按钮事件处理的方法
    private void initButtonEvents(FactoryPanel factoryPanel) {
        // 进入用户中心界面事件
        ...
        rent.addActionListener(e -> {
            // 获得正在进行的订单
            List<Order> order1 = getOrder(Serve.Order.ORDER_ING.getStatus());
            if (!order1.isEmpty()) {
                // 已经租凭了充电宝自动跳转到订单详情
                // 开启订单订单计时
                startOrderPanel();
    ...
    private void setMapVIsible() {
        ...
        // 设置要显示的充电柜的对象与数量
        List<PowerBankCabinet> powerBankCabinets = observerCabinet.getCabinets(4);
        ...
        for (int e = start; e < end; e++) {
            ...
            for (int i = 0; i < cabinetCount; i++) {
                ...
                if (e == nu) {
                    ...
                    //保存拥有充电柜信息的标签，用于用户选择充电柜后切换标签样式
                    cabinetsLabels[count] = addressLabels[e];
    ...
    private List<Order> getOrder(String status) {
        OrderSever createOrderSever = new OrderSever();
        return createOrderSever.getIngOrder(frame.getUser().getNameID(), status);
    }
    ...
    private void updataOnlineUser() {
        User user = new UserCommonLogin().upDataUserMessage(frame.getUser().getNameID());
        frame.setUser(user);
    }
}